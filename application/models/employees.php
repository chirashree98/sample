...
class employees extends CI_Model {
	...
	public function portfolios_image($id){
		$query=$this->db->where('portfolio.portfoilo_id',$id)->get('portfolio')->row();
		 $rows=$query->portfolio_image;
		$val =explode(',',$rows);
		$path = $_SERVER['DOCUMENT_ROOT'].'/employeeregistrationscookie/c6218345321/'.$val[0];
		unlink($path);
		$this->db->select('portfolio.portfolio_image');
		//$this->db->delete('portfolio',array('portfolio.portfoilo_id'=>$id));
	}
